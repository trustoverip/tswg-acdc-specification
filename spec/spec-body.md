...
To elaborate, using blindable state update events in a Registry, the SAID of the ACDC is not guessable by a 3rd party and therefore not linkable to its Registry SAID (TEL) nor to the SAID of any transaction event in that Registry (TEL). It needs to be disclosed to that 3rd party.  To clarify, a 3rd party can't guess a Registry identifier (SAID) or transaction event SAID that corresponds to a given ACDC; it would first have to be disclosed to them. 
