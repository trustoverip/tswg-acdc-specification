...
To elaborate, when the SAID of each Registry (TEL) and the SAID of each transaction event in that TEL are computed over high-entropy UUIDs, the values themselves are not guessable. A 3rd party would need to know the SAID value in order to query a Registrar or Observer for the appropriate Registry ID to get the SAID of a given transaction update event. Indeed, using high-entropy Registry identifiers (SAIDs) means that the database of Registries appears to a 3rd party as a dead drop. A 3rd party can't guess a Registry identifier that corresponds to a given ACDC; it would first have to be disclosed to them. 

Likewise, because the SMT provides herd privacy with respect to which transaction event updates belong to which bulk-issued sets and their corresponding Registries, a pair of malicious 2nd parties cannot compare inclusion proofs to determine if two ACDCs belong to the same bulk-issued set. Moreover, because each ACDC in a bulk-issued set gets its own Registry, the registry database does not provide a point of correlation between registries that belong to the same bulk-issued set of ACDCs. Therefore, a given disclosure of a bulk-issued ACDC does not directly disclose any points of correlation between malicious 2nd parties.
